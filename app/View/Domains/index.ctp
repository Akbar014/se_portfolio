                    ...
                    <tbody>
                        <?php foreach ($domains as $domain): ?>
                            <tr>
                                ...
                                <td><?php echo h($domain['Domain']['purchase_date']); ?>&nbsp;</td>
                                <td>
                                    <?php
                                    echo date('Y-m-d', strtotime($domain['Domain']['purchase_date']) + ($domain['Domain']['for_year'] * 365 * 24 * 60 * 60));
                                    ?>
                                </td>
                                <td><?php echo h($domain['Domain']['price']); ?>&nbsp;</td>
                                <td><?php echo h($domain['Domain']['exchange_rate']); ?>&nbsp;</td>
                                <td><?php echo h($domain['Domain']['paid']); ?>&nbsp;</td>
                                <td><?php echo $status[$domain['Domain']['status']]; ?>&nbsp;</td>
                                <td><?php echo h($domain['Domain']['created']); ?>&nbsp;</td>
                                <td class="actions">
                                    <?php echo $this->Html->link(__("<i style='color:green' class='fa fa-eye'></i>"), array('action' => 'view', $domain['Domain']['id']), array('escape' => false, 'title' => 'View')); ?>
                                    <?php echo $this->Html->link(__("<i style='color:black' class='fa fa-edit'></i>"), array('action' => 'edit', $domain['Domain']['id']), array('escape' => false, 'title' => 'Edit')); ?>
                                    <?php echo $this->Form->postLink(__("<i style='color:red' class='fa fa-trash-o'></i>"), array('action' => 'delete', $domain['Domain']['id']), array('escape' => false, 'title' => 'Delete', 'confirm' => __('Are you sure you want to delete # %s?', $domain['Domain']['id']))); ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
