        ...
        <div class="col-lg-12">
            <div class="panel panel-green">
                <div class="panel-heading text-center bold-text">
                    <div class="pull-left"><h4><?php echo __('Loans'); ?></h4></div>
                    ...
                    <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            <?php foreach ($lends as $lend): ?>
                                <tr>
                                    <td><?php echo h($lend['Lend']['id']); ?></td>
                                    <td>
                                        <?php echo $this->Html->link($lend['Borrower']['name'], array('controller' => 'borrowers', 'action' => 'view', $lend['Borrower']['id'])); ?>
                                    </td>
                                    <td><?php echo h($lend['Lend']['date']); ?></td>
                                    <td><?php echo h($lend['Lend']['amount']); ?></td>
                                    <td><?php echo h($lend['Lend']['remark']); ?></td>
                                    <td><?php echo $status[$lend['Lend']['status']]; ?></td>
                                    <td class="actions">
                                        <?php echo $this->Html->link(__('<i style="color:green" class="glyphicon glyphicon-unchecked" title="View"></i>'), array('action' => 'view', $lend['Lend']['id']), array('escape' => false, 'target' => '_blank')); ?>
                                        <?php echo $this->Html->link(__('<i style="color:black" class="glyphicon glyphicon-edit" title="Edit"></i>'), array('action' => 'edit', $lend['Lend']['id']), array('escape' => false)); ?>
                                        <?php echo $this->Form->postLink(__('<i style="color:red" class="glyphicon glyphicon-trash" title="Delete"></i>'), array('action' => 'delete', $lend['Lend']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $lend['Lend']['id']), 'escape' => false)); ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
